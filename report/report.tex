...
\author{\IEEEauthorblockN{Giovanni Rebecchi}
\IEEEauthorblockA{Department of Information Engineering and Computer Science\\
University of Trento, Italy\\
Student ID: \texttt{<<<INSERT STUDENT ID>>>} \quad
Email: \texttt{<<<INSERT UNITN EMAIL>>>}}}
...
the naive kernel and $68\%$ of the estimated single-core FMA peak, which is
what makes the communication terms visible at all. The cost model predicts
...
movement, and the amount of data moved is decided almost entirely by how the
operands are laid out across processes.
...
tiled and packed kernels at one thread. The packed kernel is $4.05\times$ the
naive one and reaches roughly 68\% of the estimated single-core FMA peak
($2.8\,\mathrm{GHz} \times 8 \times 2$). This matters beyond the headline
number: at $9.8$~Gflop/s the model's compute term dominates communication by
two orders of magnitude, so the planner would be ranking configurations on a
fraction of a percent of predicted runtime. Fixing the kernel is a
\emph{precondition} for the decomposition to be measurable at all.
...
Fig.~\ref{fig:shapes} compares the planner against a fixed $2\times2$ grid. On
short-fat operands ($M{=}N{=}512$, $K{=}32768$) it selects
$1\!\times\!1\!\times\!4$ --- pure $k$-parallelism --- and is $1.36\times$
faster ($0.2277\to0.1669$~s). For $M \ll N$ it selects $1\!\times\!4\!\times\!1$
($1.24\times$), for $M \gg N$ $4\!\times\!1\!\times\!1$ ($1.12\times$), and
$2\!\times\!2\!\times\!1$ for tall-skinny ($1.06\times$), reproducing
$P_r=\sqrt{PM/N}$ from data rather than from the formula. These are
decompositions a fixed 2D grid cannot reach at all.
...
than the fixed grid. The cause is visible in the model itself: at this size the
...
Table~\ref{tab:bsweep} explains why. The model says only the latency term
depends on $b$; the measured latency term is indeed negligible ($21\,\mu$s at
$b{=}256$). What actually varies with $b$ is the \emph{compute} time, which
falls from $224$~ms at $b{=}16$ to $119$~ms at $b{=}256$, because narrow panels
mean short, inefficient kernel invocations. The textbook reading of $b$ as a
pure latency knob is therefore wrong on this machine: $b$ is dominated by local
kernel efficiency, and a model that omits that term mispredicts by a third.
This is a concrete limitation of~(\ref{eq:model}) that our measurements
expose.
...
\textbf{Scaling, engines and replication.} Within one node, strong scaling at
$n{=}2048$ gives speedups $1.00/1.92/3.55$ at $P{=}1,2,4$ --- 89\% efficiency at
four processes, $124.5$~Gflop/s. This is a \emph{shared-memory} result: it
...
recovers $1.06\times$--$1.36\times$ over a fixed near-square grid across skewed
shapes, discovering decompositions such as $1\!\times\!1\!\times\!4$ that a
fixed 2D grid cannot express. The model predicts measured runtime within 20\%
...
bandwidth-bound regime; MPI-4 persistent collectives as a third broadcast
policy; a network roofline placing configurations by
